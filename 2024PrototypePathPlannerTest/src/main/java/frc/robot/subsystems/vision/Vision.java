...
public class Vision extends SubsystemBase {
  ...
  public Vision() {
    ...
    //NetworkTable PhotonVision info
    table = NetworkTableInstance.getDefault().getTable("photonvision");
    //Rotation PID Controller
    rotationPID = new PIDController(.01, .02, .001);
    rotationPID.setTolerance(.25); // allowable angle error
    rotationPID.enableContinuousInput(0, 360); // it is faster to go 1 degree from 359 to 0 instead of 359 degrees
