...
public class Vision extends SubsystemBase {
  ...
  public Vision() {
    ...
    // Shuffleboard Tab.
    createShuffleboard();
    SmartDashboard.putNumber("Vision Center Offset", 0.0);
  }
    ...
    if (result.hasTargets()) {
      ...
      if (targets.size() == 1) {
        ...
        for (int i = 0; i < targets.size(); i++) {
          totalYaw += targets.get(i).getYaw();
        }
        totalYaw /= targets.size();
        double offset = 0.0;
        offset = SmartDashboard.getNumber("Vision Center Offset", 0.0);
        totalYaw += offset;
        return -1.0 * rotationPID.calculate(totalYaw);
      }
    } else {
      return 0.0;
    }
