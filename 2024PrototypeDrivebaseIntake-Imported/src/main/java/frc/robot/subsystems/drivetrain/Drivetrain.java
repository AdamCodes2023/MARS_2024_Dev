...
import frc.lib.team3061.vision.VisionConeCube;
import frc.lib.team3061.vision.VisionTape;
...
public class Drivetrain extends SubsystemBase {
  ...
  public Translation2d centerGravity;
  ...
  public Pose2d estimatedPoseWithoutGyro = new Pose2d();
  ...
  private static final String SUBSYSTEM_NAME = "SwerveDrivetrainInfo";
  ...
  private PhotonCamera visionTapeCamera;

  public VisionTape visionTapeCalc;
  public VisionConeCube visionConeCubeCalc;

  private PIDController chargingPadPIDController;
  ...
  public Drivetrain(
    ...
    // this.power = new PowerDistribution(1, ModuleType.kRev);
    ...
    this.visionTapeCamera = new PhotonCamera(VISION_TAPE_CAMERA_NAME);

    this.visionTapeCalc = new VisionTape(this.visionTapeCamera);
    this.visionConeCubeCalc = new VisionConeCube();

    this.centerCharge = false;

    this.chargingPadPIDController =
        new PIDController(
            AUTO_DRIVE_P_CONTROLLER * 0.1, AUTO_DRIVE_I_CONTROLLER, AUTO_DRIVE_D_CONTROLLER);
    chargingPadPIDController.reset();
    // FIXME: SET THE TOLERANCE FOR BALANCING ON THE CHARGING STATION
    // chargingPadPIDController.setTolerance(0.4);

    ShuffleboardTab tabMain = Shuffleboard.getTab("MAIN");
    tabMain.addNumber("Gyroscope Angle", () -> getRotation().getDegrees());
    tabMain.addNumber("Pitch Angle", () -> getPitchRotation().getDegrees());
    tabMain.addBoolean("X-Stance On?", this::isXstance);
    tabMain.addBoolean("Field-Relative Enabled?", () -> this.isFieldRelative);
    // tabMain.addNumber("TOTAL CURRENT", this::getCurrent);
  ...
  */
  /**
   * Zeroes the pitch of the gyroscope. This sets the current pitch of the robot to zero degrees.
   */
  ...
  /**
   * Sets the pitch of the robot to the specified value.
   *
   * @param expectedPitch the pitch of the robot (in degrees)
   */
  public void setPitchOffset(double expectedPitch) {
    this.pitchOffset = expectedPitch - gyroInputs.pitchDeg;
  }
  ...
  /** Centers the robot on the Charging Station using PID values and the Pigeon IMU. */
  public void centerOnChargingPad() {
    // double error = 0.0 - getPitchRotation().getDegrees();
    double forwardSpeed;
    forwardSpeed =
        MathUtil.clamp(
            chargingPadPIDController.calculate(getPitchRotation().getDegrees(), 0.0), -1.0, 1.0);
    SmartDashboard.putNumber("Forward Speed", forwardSpeed);
    /*
    // FIXME: Adjust threshold for flat charging station.
    if (error < -3.0 && error > 3.0) {
      forwardSpeed = error * AUTO_DRIVE_P_CONTROLLER;
    } else {
      forwardSpeed = 0.0;
    }
    chassisSpeeds = new ChassisSpeeds((forwardSpeed * 0.5), 0.0, 0.0);
    */
    if (forwardSpeed < 0.4 && forwardSpeed > -0.4) {
      forwardSpeed = 0.0;
      centerCharge = true;
    }
    chassisSpeeds = new ChassisSpeeds((-forwardSpeed * 0.8), 0.0, 0.0);
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
    // chargingPadPIDController.reset();
  }

  public void centerOnLowNode() {
    // visionTapeCamera.setPipelineIndex(1);
    chassisSpeeds =
        new ChassisSpeeds(
            (visionTapeCalc.calculateDistanceToLowTarget()),
            0.0,
            (visionTapeCalc.calculateAngleToLowTarget()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }

  public void centerOnMidNode() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionTapeCalc.calculateDistanceToMidTarget()),
            0.0,
            (visionTapeCalc.calculateAngleToMidTarget()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }

  public void centerOnHighNode() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionTapeCalc.calculateDistanceToHighTarget()),
            0.0,
            (visionTapeCalc.calculateAngleToHighTarget()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }
  ...
  public void centerOnAprilTag() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionTapeCalc.calculateDistanceToAprilTag()),
            0.0,
            (visionTapeCalc.calculateAngleToAprilTag()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }
  ...
  public void centerOnCone() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionConeCubeCalc.calculateDistanceToCone()),
            0.0,
            (visionConeCubeCalc.calculateAngleToCone()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }
  ...
  public void centerOnCube() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionConeCubeCalc.calculateDistanceToCube()),
            0.0,
            (visionConeCubeCalc.calculateAngleToCube()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }
  ...
  public void centerOnObject() {
    chassisSpeeds =
        new ChassisSpeeds(
            (visionConeCubeCalc.calculateDistanceToObject()),
            0.0,
            (visionConeCubeCalc.calculateAngleToObject()));
    SwerveModuleState[] states = KINEMATICS.toSwerveModuleStates(chassisSpeeds, centerGravity);
    setSwerveModuleStates(states);
  }

  public boolean atXTranslation(double xPos, double originalX, boolean reset) {
    if (xPos < originalX) {

      if (poseEstimator.getEstimatedPosition().getX() <= xPos * 0.3048) {
      //if (estimatedPoseWithoutGyro.getX() <= xPos * 0.3048) {

        if (reset == true) {
          // zeroGyroscope();
          // resetPose();
          // m_odometry.resetPosition(originalPose, new Rotation2d(0.0));
        }

        return true;
      }

    } else if (poseEstimator.getEstimatedPosition().getX() >= xPos * 0.3048) {
  ...
  public boolean atYTranslation(double yPos, boolean reset) {
    if (yPos < 0) {

      if (poseEstimator.getEstimatedPosition().getY() <= yPos * 0.3048) {
    ...
    } else if (poseEstimator.getEstimatedPosition().getY() >= yPos * 0.3048) {
  ...
  public boolean atRotationAngle(double angle, boolean reset) {
    if (angle < 0) {

      if (poseEstimator.getEstimatedPosition().getRotation().getDegrees() <= angle - 2) {
    ...
    } else if (poseEstimator.getEstimatedPosition().getRotation().getDegrees() >= angle - 2) {
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber("PoseX", poseEstimatorPose.getX());
  }
